...
function it($m,$p){echo"\e[3",(is_callable($p)?$p():$p)?'2m✅':'1m❌'.register_shutdown_function(function () {
        return die(1);
    })," It $m\e[0m\n";}
function all(array$l){return array_reduce($l, function ($a, $p) {
    return $a && (is_callable($p) ? $p() : $p);
},1);}
